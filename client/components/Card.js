...
const Card = ({ it, currentRate, currencyType, itemQuantityInBasket }) => {
  ...
  const { id, title, image, price } = it
  return (
    <article className="overflow-hidden rounded-lg shadow-lg">
      <img alt={title} className="block object-cover h-64 w-full" src={image} loading="lazy" />
      ...
      <div className="container mx-auto flex justify-between p-5 flex-col md:flex-row items-center sm:px-2">
        <div>
          <span className="mx-1">{(price * currentRate).toFixed(2)}</span>
        ...
        <div className="sm:px-2">
          {!itemQuantityInBasket && (
            <button
              ...
              onClick={() => dispatch(changeItemQuantityInBasket(id, 1))}
            >
              Add to cart
            </button>
          )}
